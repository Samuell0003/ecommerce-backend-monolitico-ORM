...
public class ImageService {
    ...
    public List<ImageVO> findAll() {
        ...
        users.stream().forEach(e -> {
            try {
                e.add(linkTo(methodOn(ImageController.class).findById(e.getIdImage())).withSelfRel());
            } catch (Exception ex) {
    ...
    public ImageVO update(ImageVO imageVO) throws Exception {
        ...
        imageRepository.findById(imageVO.getIdImage()).orElseThrow(() -> new ResourceNotFoundException("No records found for this ID."));
        Image image = DozerMapper.parseObject(imageVO, Image.class);
    ...
    public void validateFields(ImageVO imageVO) throws Exception {
        if (imageVO.getDescription().isBlank() || imageVO.getDescription().isEmpty() 
            || imageVO.getUrl().isBlank() || imageVO.getUrl().isEmpty()) {
